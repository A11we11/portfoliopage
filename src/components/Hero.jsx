    ...
    >
      ...
      <div className="relative z-10 max-w-6xl mx-auto px-6 text-center h-full flex flex-col justify-center">
        ...
        >
          {/* Greeting */}
          <div className="mb-4">
            <span className="inline-block px-4 py-2 bg-foreground text-background rounded-full text-sm font-medium animate-fade-in">
              {heroData.greeting}
            </span>
          </div>
          {/* Name */}
          <h1 className="text-5xl md:text-7xl leading-tight font-bold mb-4 font-oswald text-primary bg-clip-text animate-fade-in-up">
            {heroData.name}
          </h1>
          {/* Typewriter Role */}
              ...
              <a
                ...
                className={`group relative px-5 py-2.5 rounded-lg  transition-all duration-300 font-medium text-sm ${
                  button.variant === "primary"
                    ? "bg-foreground text-background shadow-lg hover:shadow-xl hover:bg-gray-800 "
                    : "border-2 border-foreground  text-foreground dark:border-2    bg-background hover:bg-foreground hover:text-background"
                }`}
              >
                {button.variant === "primary" && (
                  <div
      ...
      bg-blue-400 rounded-full opacity-60 animate-float
      "
      ></div>
      <div
        className="absolute top-40 right-20 w-6 h-6 
      bg-purple-400 rounded-full opacity-60 animate-float
      delay-1000"
      ></div>
      <div
        className="absolute bottom-40 left-20 w-3 h-3 
      bg-green-400 rounded-full opacity-60 animate-float
      delay-2000"
      ></div>
      <div
        className="absolute bottom-20 right-10 w-5 h-5 
      bg-orange-400 rounded-full opacity-60 animate-float
      delay-3000"
      ></div> */}
    </section>
  );
